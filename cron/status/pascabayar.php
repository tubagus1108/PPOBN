...
$check_order = mysqli_query($db, "SELECT * FROM orders_pascabayar WHERE status IN ('','Pending','Processing') AND provider = 'DIGIFLAZZ'");

if (mysqli_num_rows($check_order) == 0) {
    die("Pesanan Pending Tidak Ditemukan.");
} else {
    while ($data_order = mysqli_fetch_assoc($check_order)) {
        $o_oid = $data_order['trx_id'];
        $phone = $data_order['data'];
        ...
        if ($o_provider == "MANUAL") {
            ...
            $check_service = mysqli_query($db, "SELECT * FROM services_pascabayar WHERE service LIKE '$service'");
            $data_service = mysqli_fetch_assoc($check_service);
            $pid = $data_service['pid'];

            if ($o_provider == "DIGIFLAZZ") {
                $sign = md5($p_id . $p_apikey . $o_oid);
            ...
            if ($o_provider == "DIGIFLAZZ") {
                ...
                $update_order = mysqli_query($db, "UPDATE orders_pascabayar SET status = '$u_status', catatan = '$u_catatan' WHERE trx_id = '$o_oid'");
                if ($update_order == true) {
                    ...
                    echo "===============<br>Status Pasca Berhasil Di Update<br><br>ID Provider : $o_oid<br>Status : $u_status<br>Keterangan : $u_catatan<br>===============<br>";
                }
            } else {
                echo "Gagal Menampilkan Data Status Pasca.";
            }
        }
    }
}
