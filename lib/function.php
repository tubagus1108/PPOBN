...
function input_request($data,$db){
    return mysqli_real_escape_string($db,stripslashes(strip_tags(htmlspecialchars($data,ENT_QUOTES))));
}

function validate_phone($input) {
	if(!preg_match('/[^+0-9]/', trim($input))) { 
		if(substr(trim($input), 0, 2) == '62') { //  Jika karakter 1 - 3 adalah 62
			$result = trim($input);
		} elseif(substr(trim($input),  0,  3) == '+62') { //  Jika karakter 1 - 3 adalah +62
			$result = '62'.substr(trim($input),  3);
		} elseif(substr(trim($input),  0,  1) == '0') { //  Jika karakter 1  adalah 0
			$result = '62'.substr(trim($input),  1);
		}
	}
	return $result;
}