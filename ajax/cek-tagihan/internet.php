...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) AND strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    if(isset($_POST['data']) && isset($_POST['type']) && isset($_POST['category'])) {
        if(!isset($_POST['data'])) exit("No direct script access allowed!");
        if(!isset($_POST['category'])) exit("No direct script access allowed!");
        if(!isset($_POST['type']) || !in_array($_POST['type'],['INTERNET PASCABAYAR'])) exit("No direct script access allowed!");
        if(empty($_POST['data'])) print '<div class="alert alert-info">Mohon masukan nomor pelanggan.</div>';
        if(empty($_POST['category'])) print '<div class="alert alert-info">Mohon pilih kategori layanan.</div>';
        ...
        $search = mysqli_query($db, "SELECT * FROM services_pascabayar WHERE type = '$post_type' AND category = '$post_category' AND status = 'Active' ORDER BY id ASC");
