...
if (isset($_POST['add'])) {
	...
	$input_name = array('level', 'password', 'full_name', 'balance', 'status', 'verifikasi');
	if (check_input($_POST, $input_name) == false) {
		$_SESSION['result'] = array('alert' => 'danger', 'title' => 'Gagal!', 'msg' => 'Input tidak sesuai.');
	} else {
		$validation = array(
			'level' => input_request($_POST['level'], $db),
		...
		if (check_empty($validation) == true) {
			...
			if (empty($_POST['password'])) {
				$input_post = array(
					'level' => input_request($_POST['level'], $db),
					'full_name' => input_request($_POST['full_name'], $db),
					'balance' => input_request($_POST['balance'], $db),
					'nomor' => input_request($_POST['nomor'], $db),
					'email' => input_request($_POST['email'], $db),
					'status' => input_request($_POST['status'], $db),
					'user_verif' => input_request($_POST['verifikasi'], $db)
				);
			} else {
				$input_post = array(
					'level' => input_request($_POST['level'], $db),
